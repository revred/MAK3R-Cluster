...
public class MazakEdgeAdapter : IEdgeConnectorAdapter
{
    private readonly ILogger<MazakEdgeAdapter> _logger;
    ...
    public MazakEdgeAdapter(EdgeConnectorConfig config, string siteId, ILogger<MazakEdgeAdapter> logger)
    {
        _config = config;
        _siteId = siteId;
        _logger = logger;

        // Create MTConnect connector configuration for Mazak
        var mtconnectConfig = new MTConnectConfig
        {
            ConnectorId = config.MachineId,
            MachineId = config.MachineId,
            Make = "MAZAK",
            BaseUrl = GetSetting<string>("BaseUrl") ?? $"http://{config.IpAddress}:5000/MAZAK",
    ...
    {
        _logger.LogInformation("Starting Mazak MTConnect Edge adapter for machine {MachineId}", MachineId);
        _isRunning = true;
    ...
    {
        _logger.LogInformation("Stopping Mazak MTConnect Edge adapter for machine {MachineId}", MachineId);
        _isRunning = false;
        await Task.CompletedTask;
    }
    ...
    private KMachineEvent? ConvertToKMachineEvent(UpsertEvent upsertEvent)
    {
        try
        {
            ...
            return new KMachineEvent
            {
                ...
                Event = new EventInfo
                {
                    Type = DetermineEventTypeForMazak(machineData),
                    Severity = "INFO"
                }
            };
        }
    ...
    private string? DetermineEventTypeForMazak(Dictionary<string, object> data)
    {
        ...
        // Mazak-specific event inference (5-axis specific)
